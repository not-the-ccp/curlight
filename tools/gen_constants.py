...
def main() -> None:
    ...
    groups = {
        "CurlOpt": "CURLOPT_", "CurlInfo": "CURLINFO_", "CurlCode": "CURLE_",
        "MultiOpt": "CURLMOPT_", "MultiCode": "CURLM_",
        "HttpVersion": "CURL_HTTP_VERSION_", "ProxyType": "CURLPROXY_",
        "SslVersion": "CURL_SSLVERSION_", "IpResolve": "CURL_IPRESOLVE_",
    }
    names = sorted(set(re.findall(
        r"\b(?:CURLOPT_|CURLINFO_|CURLE_|CURLMOPT_|CURLM_|CURL_HTTP_VERSION_|"
        r"CURLPROXY_|CURL_SSLVERSION_|CURL_IPRESOLVE_)[A-Z0-9_]+\b", text
    )))
    ...
    for group, prefix in {
        "CurlAuth": "CURLAUTH_", "Protocol": "CURLPROTO_",
        "SslOpt": "CURLSSLOPT_", "CurlPause": "CURLPAUSE_",
    }.items():
        groups[group] = prefix
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        subprocess.run([os.environ.get("CC", "cc"), *flags, "-Wno-deprecated-declarations",
                        str(path / "probe.c"), "-o", str(path / "probe")], check=True)
        values = dict(line.split() for line in subprocess.check_output(
            [str(path / "probe")], text=True).splitlines())
    lines = ['"""Generated libcurl constants. Run tools/gen_constants.py to regenerate."""',
             "", "from enum import IntEnum, IntFlag", ""]
    for group, prefix in groups.items():
        base = "IntFlag" if group in {"CurlAuth", "Protocol", "SslOpt", "CurlPause"} else "IntEnum"
        lines += ["", f"class {group}({base}):", f'    """{prefix} constants from libcurl headers."""', ""]
